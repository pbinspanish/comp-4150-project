    ...
    try {
        ...
        if ($employee) {
            ...
            echo '  </select>
                    ...
                    <button type="submit" class="btn btn-success">Update Job Title</button>
                </form>';
        } else {
        ...
        if (isset($_POST['job_title']) && isset($_POST['employee_id'])) {
            
            $update_sql = "UPDATE employees SET title_id = ? WHERE employee_id = ?";
            $update_stmt = $pdo->prepare($update_sql);
            $update_stmt->execute([$_POST['job_title'], $_POST['employee_id']]);
            
            echo '<div class="alert alert-success mt-3">Job title updated successfully.</div>';
        }
